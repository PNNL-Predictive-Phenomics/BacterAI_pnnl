"""
Diagnostic and visualisation utilities for omicsTL Bayesian optimisation.

Three main plots are provided:

plot_candidate_scores
    Scatter of predicted mean vs uncertainty, coloured by acquisition score,
...
def plot_candidate_scores(
    ...
    """Predicted mean vs uncertainty, coloured by acquisition score.

    Parameters
    ----------
    candidates : full scored candidate pool returned by
                 ``recommend_next_batch(..., return_candidates=True)``.
    batch      : recommended batch DataFrame (output of recommend_next_batch).
                 If provided, batch points are overlaid as red stars.
...
# ── 2. Acquisition landscape (2-D heatmap) ────────────────────────────────────
...
def plot_acquisition_landscape(
    ...
    Parameters
    ----------
    model_info   : ``{'model': ..., 'type': 'dl' | 'rf'}``.
    existing_data : observations so far (must contain feature_cols + response_col).
    response_col  : name of the response column.
    feature_cols  : full feature column list (must match what the model expects).
    x_col, y_col  : the two features to plot on the axes.
    batch         : if provided, recommended batch points are overlaid.
    feature_ranges : ``{col: (min, max)}``.  Defaults to observed range.
    n_grid        : grid resolution per axis (n_grid² points scored).
    acquisition   : 'EI' (default), 'UCB', or 'POI'.
    kappa, xi     : acquisition hyper-parameters (same semantics as
                    ``recommend_next_batch``).
    n_mc_samples  : MC Dropout passes for DL models.  Lower than the default 50
                    is fine here since we only need a visual surface.
    figsize       : figure size in inches.
    ...
    # Fix context features at the best-observed row
    best_row = existing_data.loc[existing_data[response_col].idxmax()]

    # Build a dense grid over x_col × y_col
    ...
    grid = pd.DataFrame(
        {col: np.full(n_pts, float(best_row[col])) for col in feature_cols}
    )
    grid[x_col] = XX.ravel()
    grid[y_col] = YY.ravel()
    ...
    mu, sigma = predict_with_uncertainty(model_info, grid, feature_cols, n_mc_samples)
...
# ── 3. Batch coverage — parallel coordinates ──────────────────────────────────
...
def plot_batch_coverage(
    ...
    response_col  : if provided, existing data lines are coloured by response
    ...
    # Normalise each feature to [0,1] using existing-data range
    mins  = existing_data[feature_cols].min()
    maxs  = existing_data[feature_cols].max()
    spans = (maxs - mins).replace(0.0, 1.0)

    def _norm(row: pd.Series) -> np.ndarray:
        return ((row[feature_cols] - mins) / spans).values.astype(float)

    # ── Existing observations ──────────────────────────────────────────────────
    if response_col is not None and response_col in existing_data.columns:
        resp = existing_data[response_col].values.astype(float)
    ...
    # ── Recommended batch ──────────────────────────────────────────────────────
    for rank_i, (_, row) in enumerate(batch.iterrows(), start=1):
    ...
    # Light vertical grid lines at each feature axis
    for xp in x_pos:
